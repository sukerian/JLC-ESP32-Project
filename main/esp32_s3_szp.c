...
{
    lcd_buffer[buf_index] = (void *)calloc(1, BSP_LCD_H_RES * BSP_LCD_V_RES * 2);
}
...
void app_lcd_draw(uint8_t *buf, uint32_t len, uint16_t width, uint16_t height)
{
    ...
    esp_lcd_panel_draw_bitmap(panel_handle, 0, 0, BSP_LCD_H_RES, BSP_LCD_V_RES, lcd_buffer[buf_index]);
